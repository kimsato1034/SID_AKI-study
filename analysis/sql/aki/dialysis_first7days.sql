...
FROM
  `physionet-data.eicu_crd.treatment` treatment
WHERE
...
FROM
  `physionet-data.eicu_crd.pasthistory` pasthistory
WHERE
...
SELECT
  patientunitstayid
, dialysis_in_the_icu_bin AS dialysis_in_the_icu_first7days_bin
, first_dialysis_offset AS first_dialysis_in_the_icu_first7days_offset
FROM
sq4
WHERE 
first_dialysis_offset BETWEEN 0 AND 60*24*7 --7 days